...
export default function EventPage() {
  ...
  useEffect(() => {
    ...
    // URL'den kodları al ve doğrula
    if (betaCode && eventCode) {
  ...
  const handleBetaAccess = async (code: string) => {
    try {
      ...
      if (valid) {
        ...
        // Beta kodu doğrulandıktan sonra etkinlik kodunu doğrula
        if (eventCode) {
          handleEventAccess(eventCode)
        }
      } else {
        toast({
          title: 'Geçersiz Beta Kodu',
  ...
  if (!isEventAccessGranted) {
    return (
      <div className="min-h-screen bg-gradient-to-br from-pink-50 to-rose-100 flex items-center justify-center">
        <div className="text-center">
          <div className="animate-spin rounded-full h-12 w-12 border-b-2 border-brand-primary mx-auto mb-4"></div>
          <p className="text-gray-600">Etkinlik kodu doğrulanıyor...</p>
        </div>
      </div>
    )
  }
